...
@Slf4j
@Singleton
class CballMakerOverlay extends OverlayPanel
{
	private final Client client;
	private final CballMakerPlugin plugin;
	private final CballMakerConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private CballMakerOverlay(final Client client, final CballMakerPlugin plugin, final CballMakerConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "c17 - CballMaker"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startCballMaker || !config.enableUI())
		{
