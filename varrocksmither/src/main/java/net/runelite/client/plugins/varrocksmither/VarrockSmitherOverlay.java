...
class VarrockSmitherOverlay extends OverlayPanel
{
	...
	@Inject
	private VarrockSmitherOverlay(final Client client, final VarrockSmitherPlugin plugin, final VarrockSmitherConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "c17 - VarrockSmither"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startVarrockSmither || !config.enableUI())
		{
